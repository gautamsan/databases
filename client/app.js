var app = {};
// app.server = 'http://127.0.0.1:3000/classes/messages' //used for all requests
app.server = 'http://127.0.0.1:3000' //used for all requests
app.friends = {};
app.intervalID;
...
app.init = function() {
  ...
  // //app.clearMessages();
  var user = app.escapeCharacters(window.location.search.slice(10))
  console.log(user);
  //Create new user in Database
  app.send({username: user}, '/classes/users');
  //Get messages for room 'Lobby'
  //app.fetch()
};
...
app.addMessage = function(message) {
  var $chats = $('#chats');
  var newChat = app.escapeCharacters(String(message.message));
...
app.handleSubmit = function (messageText) {
  ...
  var message = {
    ...
    message: app.escapeCharacters(messageText),
...
app.getMessages = function(roomName) {
  ...
  }, '/classes/messages?order=-createdAt&where={"roomname":"'+ app.currentRoom +'"}');
}
